...
public:
	...
	void MovemodleLR(float speed);
	void MovemodleFB(float speed);
	void Setspeed(float speed){m_mspeed=speed;}
	float Getspeed(){return m_mspeed;}
	Vector3 GetmPosition(){return m_mPosition;}
private:
	/** 读一个字符串 */
	int  GetString(char *);								
